...
namespace LectureSchedulingTool.Controllers
{
    public partial class SchedulingController : Controller
    {
        ...
        public ActionResult Students_group_load(SVM.Students_group_load model, int page = 1, char action = '0', int row = -1, int id_students_group_load = -1)
        {
            switch (action)
            {
                ...
                case 's':
                    if (ModelState.IsValid)
                    {
                        if (DB.Students_group_load.Count(sgl => sgl.id_students_group == model.id_students_group && sgl.id_subject == model.id_subject) > 0)
                        {
                            ...
                            ModelState.AddModelError("-2", Localizator.Localizate("Students_group_load_duplicate_group_and_subject_error", CurrentLangCode));
                        }
                        else
                        {
                            ViewBag.action = '0';
                            ...
                            {
                                ModelState.AddModelError("-3", Localizator.Localizate("Students_group_load_add_error", CurrentLangCode));
                ...
                case 'u':
                    if (ModelState.IsValid)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                ModelState.AddModelError("-3", Localizator.Localizate("Students_group_load_edit_error", CurrentLangCode));
                                if (HttpContext.IsDebuggingEnabled)
                                    ModelState.AddModelError("-3", ex.GetBaseException().Message);
                            }
                        }
                ...
                case 'r':
                    ...
                    if (DB.Students_group_load.Count(sgl => sgl.id_students_group == model.id_students_group && sgl.id_subject == model.id_subject) > 0)
                    {
                        ...
                        {
                            ModelState.AddModelError("-4", Localizator.Localizate("Students_group_load_delete_error", CurrentLangCode));
                            if (HttpContext.IsDebuggingEnabled)
                    ...
                    {
                        ModelState.AddModelError("-5", Localizator.Localizate("Students_group_load_existance_error", CurrentLangCode));
                    }
                    break;
            ...
            {
                ModelState.AddModelError("-1", Localizator.Localizate("Students_group_load_view_error", CurrentLangCode));
                if (HttpContext.IsDebuggingEnabled)
