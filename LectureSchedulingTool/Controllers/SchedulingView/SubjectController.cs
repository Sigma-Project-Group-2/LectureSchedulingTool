...
namespace LectureSchedulingTool.Controllers
{
    ...
    {
        [Authorize]
        public ActionResult Subject(Subject model, int page = 1, char action = '0', int row = -1, int id_subject = -1)
        {
            switch (action)
            {
                case 'a':
                ...
                case 'u':
                    if (ModelState.IsValid)
                    {
                        if (DB.Subject.Count(s => s.name == model.name && s.type == model.type && s.id_department != model.id_department) > 0)
                        {
                            ViewBag.action = 'e';
                            ViewBag.row = row;
